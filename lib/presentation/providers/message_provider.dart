...
class MessageProvider extends ChangeNotifier {
  ...
  int get pendingMessages =>
      _messages.where((m) => m.status == MessageStatus.pending).length;
  int get sentMessages =>
      _messages.where((m) => m.status == MessageStatus.sent).length;
  int get deliveredMessages =>
      _messages.where((m) => m.status == MessageStatus.delivered).length;
  ...
  Future<void> receiveMessage(MeshMessage message) async {
    try {
      ...
      Logger.info(
        'Received message: ${message.messageId} from ${message.sourceId}',
      );
      notifyListeners();
  ...
  void deleteConversation(String contactId) {
    ...
    _messages.removeWhere(
      (m) => m.sourceId == contactId || m.destinationId == contactId,
    );
    ...
    if (removedCount > 0) {
      ...
      Logger.info(
        'Deleted conversation with $contactId ($removedCount messages)',
      );
      notifyListeners();
    }
  }
  ...
  void _updateConversationCache(String contactId, MeshMessage message) {
    if (_conversationCache.containsKey(contactId)) {
      ...
      final existingIndex = conversation.indexWhere(
  ...
  Future<void> _sendAcknowledgment(MeshMessage originalMessage) async {
    try {
      // TODO: Implement ACK message sending through Bluetooth service
      // This will be handled by the network layer after route establishment
      Logger.info('ACK required for message: ${originalMessage.messageId}');
    } catch (error, stackTrace) {
